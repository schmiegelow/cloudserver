...
const multipleBackendGateway = require('./external/multipleBackendGateway');
...
function _put(cipherBundle, value, valueSize, keyContext, backendInfo,
log, cb) {
    ...
    if (implName === 'multipleBackends') {
        ...
        return client.put(hashedStream, valueSize, keyContext, backendInfo,
...
const data = {
    ...
    */
    ...
    destBackendInfo, log, cb) =>
        data.put(cipherBundle, stream, part.size, dataStoreContext,
            ...
            if (part.dataStoreType === 'azure') {
                ...
                return async.parallel([
                    ...
                    parallelCb => data._dataCopyPut(serverSideEncryption,
                        passThrough, part, dataStoreContext, destBackendInfo,
                        log, parallelCb),
                ], (err, res) => {
                    if (err) {
                        return copyCb(err);
    ...
    dataStoreContext, destBackendInfo, locations, log, cb) => {
        ...
        .locationConstraints[destLocationConstraintName].type]) {
            return multipleBackendGateway.uploadPart(null, null,
            stream, numberPartSize, destLocationConstraintName, destObjectKey,
            uploadId, partNumber, destBucketName, log,
            (err, partInfo) => {
                if (err) {
        ...
        if (serverSideEncryption) {
            ...
            (err, cipherBundle) => {
                ...
                return data.put(cipherBundle, stream, numberPartSize,
                ...
                if (part.dataStoreType === 'azure') {
                    ...
                    return async.parallel([
                        ...
                        next => data._dataCopyPutPart(request,
                            ...
                            dataStoreContext, backendInfo, locations, log,
                            next),
                    ], err => {
                        if (err) {
                            return cb(err);
